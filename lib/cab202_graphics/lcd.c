 ...
 *  Last modified Zacharia Henderson, 18/05/2021 12:45 AM
 *
 */
...
void lcd_init(uint8_t contrast) {
	...
	SET_OUTPUT(SCEPIN_DDRD, SCEPIN);
	SET_OUTPUT(RSTPIN_DDRD, RSTPIN);
	SET_OUTPUT(DCPIN_DDRD, DCPIN);
	SET_OUTPUT(DINPIN_DDRD, DINPIN);
	SET_OUTPUT(SCKPIN_DDRD, SCKPIN);

	CLEAR_BIT(RSTPIN_PORT, RSTPIN);
	SET_BIT(SCEPIN_PORT, SCEPIN);
	SET_BIT(RSTPIN_PORT, RSTPIN);
...
void lcd_write(uint8_t dc, uint8_t data) {
	...
	WRITE_BIT(DCPIN_PORT,DCPIN,dc);

	// Pull the SCE/SS pin low to signal the LCD we have data
	CLEAR_BIT(SCEPIN_PORT,SCEPIN);

	// Write the byte of data using "bit bashing"
	for(int i = 7; i >= 0; i--) {
		CLEAR_BIT(SCKPIN_PORT, SCKPIN) ;
		if((data>>i) & (1 == 1)) {
			SET_BIT(DINPIN_PORT, DINPIN);
		} else {
			CLEAR_BIT(DINPIN_PORT, DINPIN);
		}
		SET_BIT(SCKPIN_PORT, SCKPIN);
	}
